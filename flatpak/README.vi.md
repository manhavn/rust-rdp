...
| File | Vai trò |
|------|---------|
| [`io.github.manhavn.rust-rdp-vnc.yml`](io.github.manhavn.rust-rdp-vnc.yml) | Manifest Flatpak |
| [`io.github.manhavn.rust-rdp-vnc.desktop`](io.github.manhavn.rust-rdp-vnc.desktop) | Desktop entry |
| [`io.github.manhavn.rust-rdp-vnc.metainfo.xml`](io.github.manhavn.rust-rdp-vnc.metainfo.xml) | Metadata AppStream (**bắt buộc** trên Flathub) |
...
flatpak-builder → cài user            PR vào flathub/flathub (nhánh new-pr)
                                      ...
                                      → user: flatpak install flathub io.github.manhavn.rust-rdp-vnc
```

App ID: **`io.github.manhavn.rust-rdp-vnc`** (giữ cố định sau khi publish).
...
Kết quả: `flathub-out/io.github.manhavn.rust-rdp-vnc/` (manifest git tag + `generated-sources.json`).

---

## Test local trước khi mở / sửa PR Flathub

**Luôn chạy local cho xanh** — CI Flathub chờ lâu, reviewers là tình nguyện.
...
# 1) Nhanh (~vài giây): linter manifest = bước GitHub Actions hay fail
./scripts/publish-flatpak.sh --lint

# 2) Đầy đủ trước PR: lint + gói offline + build kiểu Flathub (lâu)
#    Cần: git tag (vd v1.0.0) đã có local, và generated-sources.json
./scripts/publish-flatpak.sh --generate-sources   # lần đầu / sau khi đổi Cargo.lock
GIT_TAG=v1.0.0 ./scripts/publish-flatpak.sh --preflight

# 3) Chỉ khi preflight OK mới push lên fork / cập nhật PR
```

| Lệnh | Bắt lỗi gì | Thời gian |
|------|------------|-----------|
| `--lint` | runtime EOL, finish-args (home…), metainfo | nhanh |
| `--preflight` | như trên + build offline + lint repo | lâu (cargo) |
| `publish-flatpak.sh` | build local từ working tree | trung bình |
...
./scripts/publish-flatpak.sh --lint
./scripts/publish-flatpak.sh --preflight
```
...
- Title ví dụ: `Add io.github.manhavn.rust-rdp-vnc`  
- Bot build app; sửa đến khi CI xanh và review pass.

### 7. Sau khi merge

- App lên Flathub (có thể trễ vài phút/giờ)  
- Bảo trì lâu dài tại **`github.com/flathub/io.github.manhavn.rust-rdp-vnc`**  
...
3. Sửa repo `flathub/io.github.manhavn.rust-rdp-vnc` (tag, commit, metainfo)  
